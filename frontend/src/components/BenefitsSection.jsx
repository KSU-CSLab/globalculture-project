    ...
    <div className="space-y-4">
      ...
      {user && (
        <div className="flex items-center space-x-2 rounded-xl bg-brand-gold/8 dark:bg-brand-gold/10 border border-brand-gold/20 p-3">
          ...
          <p className="text-xs font-bold text-brand-gold-dark dark:text-brand-gold">
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center flex-wrap gap-1.5 mb-2">
                        ...
                        {benefit.eligibleRoles?.map((r) => (
                          <span key={r} className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-extrabold bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300">
                            {ROLE_LABELS[r]}
                          </span>
                        ))}
                      </div>
                      ...
                      <p className="mt-1 text-xs text-slate-500 dark:text-slate-400 leading-relaxed line-clamp-2">{benefit.description}</p>

                      {/* Meta */}
                      <div className="mt-3 flex flex-wrap gap-3 text-xs text-slate-400 dark:text-slate-500 font-bold">
                        <span className="flex items-center space-x-1">
                          <Tag size={11} /><span className="text-brand-gold-dark dark:text-brand-gold font-extrabold">{benefit.discount}</span>
                        </span>
                        <span className="flex items-center space-x-1">
                          <Users size={11} /><span>{benefit.claimedCount || 0}명 수령</span>
                        </span>
                        <span className="flex items-center space-x-1">
                          <Clock size={11} /><span>~{benefit.validUntil}</span>
                        </span>
                      </div>
                      <p className="mt-1 text-[10px] font-bold text-slate-400">제공: {benefit.partnerName}</p>

                      {/* Claimed Coupon Code */}
                      {claimed && (
                        <div className="mt-3 flex items-center space-x-2 rounded-xl bg-brand-gold/10 border border-brand-gold/30 px-3 py-2 fade-in">
                          <span className="text-xs font-mono font-extrabold text-brand-gold-dark dark:text-brand-gold tracking-widest">{claimed}</span>
                    ...
                    <div className="flex-shrink-0">
                      <button
                        ...
                        className={`flex items-center space-x-1.5 rounded-xl px-3 py-2 text-xs font-bold transition-all active:scale-95 whitespace-nowrap ${
                          claimed
